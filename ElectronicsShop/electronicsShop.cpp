...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
     ...
     */
     ...
     for (int i = 0; i < keyboards.size(); i++)
     {
         ...
         {
            if (j+1 <= drives.size())               //out of bounds check
            {
                result = keyboards[i] + drives[j];  //sum
                
                if (result >= max && result <= b)   //test if highest price without going above budget
                {
                    max = result;
                }
            }
     ...
     if (max == 0)  //if no sum below budget found
     {
        return -1;
     }
     else
