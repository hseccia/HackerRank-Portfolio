...
int birthdayCakeCandles(vector<int> candles) 
{
    ...
    {
        if (i+1 < candleSize) //prevent out of bounds error
        {
            if (candles[i] >= candles[i+1] && candles[i] > maxNum)	//if current candle is larger or equal to next candle and it is the current highest number
																	//first condition is to dynamically set initial maxNum
            {
    ...
    for (int i = 0; i < candleSize; i++)							//count instances of maxNum
    {
        if (candles[i] == maxNum)
        {
            candleAmount++;
